...
Source code: https://github.com/satijalab/seurat/blob/master/vignettes/atacseq_integration_vignette.Rmd
Author: Shicong Wang
output:
  html_document:
    theme: united
...
# install.packages("http://seurat.nygenome.org/src/contrib/pbmcMultiome.SeuratData_0.1.0.tar.gz", repos = NULL, type = "source")
...
ggsave(filename = "./atacseq_integration_vignette.jpg", height = 7, width = 12, plot = plot, quality = 50)
```
...
write.csv(x = t(as.data.frame(all_times)), file = "./atacseq_integration_vignette.csv")
```
